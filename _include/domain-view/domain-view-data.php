<?php
$label = [
    "Domain ID",
    "Ημερομηνία Κατοχύρωσης",
    "Ημερομηνία Λήξης",
    "Κατάσταση",
];
$labelval = ["201173", "16/04/2021", "16/04/2023", "ΚΑΤΑΧΩΡΗΜΕΝΟ"];

$colorClass = ["", "", "", "success"];

$nservers = [
    [
        "group" => "Θα δηλώσω νέους εξυπηρετητές",
        "option" => [
            [
                "name" => "Εισαγωγή νέων εξυπηρετητών",
                "value" => "nons",
            ],
        ],
    ],
    [
        "group" => "Θα δηλώσω εξυπηρετητές της DNHOST",
        "option" => [
            [
                "name" => "DNHOST nameservers",
                "value" => "4",
            ],
        ],
    ],
    [
        "group" => "Nameserver groups",
        "option" => [
            [
                "name" => "pantazistestdns",
                "value" => "305",
            ],
            [
                "name" => "nsgroup1",
                "value" => "419",
            ],
            [
                "name" => "adffggg9246hhrrrgfggjkjhhjkhgfjdkdfk1",
                "value" => "547",
            ],
            [
                "name" => "aaaaaa",
                "value" => "564",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaa",
                "value" => "565",
            ],
            [
                "name" => "ssssssssssssssssss",
                "value" => "566",
            ],
            [
                "name" => "asdrewsd",
                "value" => "567",
            ],
            [
                "name" => "ldkfldkfldkfldkfldkfldkldkfldkf",
                "value" => "568",
            ],
            [
                "name" => "ffffffffffffffffffffffffffff123",
                "value" => "569",
            ],
            [
                "name" => "hfhgjgjdkfdfkfkgjdkdjfkgj",
                "value" => "570",
            ],
            [
                "name" => "dffghhgfffd",
                "value" => "571",
            ],
            [
                "name" => "aaksldkslakdlakdlakdlfk",
                "value" => "572",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaaaaaa123weee",
                "value" => "587",
            ],
            [
                "name" => "aaaaaaaaaaaaa",
                "value" => "588",
            ],
            [
                "name" => "ppppppppppppppppppppp",
                "value" => "589",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                "value" => "590",
            ],
            [
                "name" => "aaaaaaaaaaaaaa",
                "value" => "591",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                "value" => "592",
            ],
            [
                "name" => "ns1.pantazis.comss",
                "value" => "593",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaa",
                "value" => "594",
            ],
            [
                "name" => "ppppppppppppppppppppppp",
                "value" => "595",
            ],
            [
                "name" => "hsgdkhskdhsdhgsd",
                "value" => "596",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                "value" => "597",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaa",
                "value" => "598",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaaaa",
                "value" => "599",
            ],
            [
                "name" => "pppppppppppppppppppppaawedf",
                "value" => "600",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa123",
                "value" => "601",
            ],
            [
                "name" => "sdhfhfjsjdksjdksfjskdjsfdksfd",
                "value" => "602",
            ],
            [
                "name" => "qqwwededssdsasddsad",
                "value" => "603",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaa111",
                "value" => "604",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaa123",
                "value" => "605",
            ],
            [
                "name" => "asfdsfgjhuioygdsrfgdhf",
                "value" => "606",
            ],
            [
                "name" => "25376487hdjskdl123qwdd1234",
                "value" => "607",
            ],
            [
                "name" => "sassasddffgghghhgffdd",
                "value" => "611",
            ],
            [
                "name" => "ddsfdfgfdsdsfdffd",
                "value" => "614",
            ],
            [
                "name" => "bbbbbbbbbbbbbbbbb",
                "value" => "616",
            ],
            [
                "name" => "aaaaaaaaa99121212",
                "value" => "618",
            ],
            [
                "name" => "aaaaaaaaaaaaanshdja",
                "value" => "619",
            ],
            [
                "name" => "ssssssssssssssssssssss12",
                "value" => "620",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaa123",
                "value" => "621",
            ],
            [
                "name" => "aaasssddfggghh",
                "value" => "622",
            ],
            [
                "name" => "asddfffggghhhj",
                "value" => "624",
            ],
            [
                "name" => "fkujdf,hdskfghsdhs1",
                "value" => "625",
            ],
            [
                "name" => "assdasdffdd",
                "value" => "626",
            ],
            [
                "name" => "ssssssssssssssssssssss123",
                "value" => "627",
            ],
            [
                "name" => "dddddddddddddddddddddddddddddd",
                "value" => "628",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaa",
                "value" => "629",
            ],
            [
                "name" => "zdsfdfgghfg",
                "value" => "630",
            ],
            [
                "name" => "ljhfjkgfhgghgh",
                "value" => "631",
            ],
            [
                "name" => "asdfghghhhhg",
                "value" => "632",
            ],
            [
                "name" => "ffffffffffffffffffffffffffffffffff",
                "value" => "635",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                "value" => "636",
            ],
            [
                "name" => "ssssssssssssssssssssss",
                "value" => "637",
            ],
            [
                "name" => "assdfgggfdsdsddsffd",
                "value" => "638",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaa",
                "value" => "639",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                "value" => "640",
            ],
            [
                "name" => "asdffghhfg",
                "value" => "641",
            ],
            [
                "name" => "sddgfdgfgfgfhghdjsjdkf",
                "value" => "642",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaaa",
                "value" => "643",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaaa123",
                "value" => "644",
            ],
            [
                "name" => "dddddddddddddddddd",
                "value" => "645",
            ],
            [
                "name" => "asdfgghjj",
                "value" => "646",
            ],
            [
                "name" => "sssssssssssssssssssssssss",
                "value" => "647",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaa1123",
                "value" => "648",
            ],
            [
                "name" => "ssdfghfdksldkjds;klfkjsddfj",
                "value" => "649",
            ],
            [
                "name" => "asssssssssssssssssssssss",
                "value" => "650",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaaaaaaa123",
                "value" => "651",
            ],
            [
                "name" => "asdfghhjjkjkk",
                "value" => "652",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaa145678",
                "value" => "653",
            ],
            [
                "name" => "aaaaaaaaaaaaaaaaaaaaaa1345657",
                "value" => "654",
            ],
            [
                "name" => "ffggghhhgf",
                "value" => "655",
            ],
            [
                "name" => "asddffghg",
                "value" => "656",
            ],
            [
                "name" => "asdfghgjgjdhdjfjfhgj",
                "value" => "657",
            ],
            [
                "name" => "hfdjgfkjfhdjkfjhgj",
                "value" => "658",
            ],
            [
                "name" => "asddfgfggff",
                "value" => "659",
            ],
            [
                "name" => "dsdssfdffsderrtgdfd",
                "value" => "660",
            ],
            [
                "name" => "dffdfffdjhfgfhjgfkjfgfd",
                "value" => "661",
            ],
            [
                "name" => "hgfhfhgjhgjfjhfjgjfgjgff",
                "value" => "662",
            ],
            [
                "name" => "gfhdjdjfhdjdhfjdjfgdkfgjdkfgj",
                "value" => "663",
            ],
            [
                "name" => "jgjhkgfjfkgjfkfjgkgjfkjgsdsdsd",
                "value" => "664",
            ],
            [
                "name" => "mghjgjfgdjhsdsdsdfg",
                "value" => "665",
            ],
            [
                "name" => "asasxdfdssdassa",
                "value" => "804",
            ],
            [
                "name" => "sddsdsdsdsdsdsdssdsdsddd",
                "value" => "805",
            ],
            [
                "name" => "sdsdsgffgdsds",
                "value" => "807",
            ],
            [
                "name" => "fgfgjjjkghhffg",
                "value" => "808",
            ],
            [
                "name" => "dsdsdssdsdfasdersddsffdfdf",
                "value" => "809",
            ],
            [
                "name" => "fgfgfgjhfgkjghngf",
                "value" => "813",
            ],
            [
                "name" => "aa",
                "value" => "815",
            ],
            [
                "name" => "dfdfdfggfgf",
                "value" => "816",
            ],
            [
                "name" => "fgfggfhggjjhhjfg",
                "value" => "820",
            ],
            [
                "name" => "pulddf",
                "value" => "821",
            ],
            [
                "name" => "dfdfddfdffdfdfdfd",
                "value" => "822",
            ],
            [
                "name" => "gfdgfggfgf",
                "value" => "823",
            ],
            [
                "name" => "gfdgfggfgf",
                "value" => "824",
            ],
            [
                "name" => "gfdgfggfgf",
                "value" => "825",
            ],
            [
                "name" => "sdsdsfds",
                "value" => "826",
            ],
            [
                "name" => "sdsddssd",
                "value" => "827",
            ],
            [
                "name" => "sdssd",
                "value" => "828",
            ],
            [
                "name" => "asaasadsdss",
                "value" => "829",
            ],
            [
                "name" => "jfdjhdfkjhfdkhgsdhkgfdg",
                "value" => "830",
            ],
        ],
    ],
];

function buttonBottom()
{
    return '<div class="addAction"><span class="icon-fi-sr-plus-small" alt=""></span>Δημιουργία νέου Προσώπου Επαφής</div>';
}

function HtmlEl($text, $classl, $color, $element, $icon = false)
{
    if ($element == "") {
        $element = "span";
    }

    $template = "";
    $iconTemplate = ' <div class="info-icon action">
  <span class="icon-fi-sr-pencil" class="button-icon" alt=""></span>
  </div>';

    if (is_array($icon)) {
        $template .=
            "<" .
            $element .
            " class='" .
            $classl .
            " " .
            $color .
            "'>" .
            $text .
            "</" .
            $element .
            ">";
        foreach ($icon as $key => $val) {
            $template .=
                '<div class="info-icon action skata' .
                $key .
                '"><span class="' .
                $val .
                '" class="button-icon" alt=""></span></div>';
        }
    }

    if ($icon == true && !is_array($icon)) {
        $template =
            "<" .
            $element .
            " class='" .
            $classl .
            " " .
            $color .
            "'>" .
            $text .
            "</" .
            $element .
            ">" .
            $iconTemplate;
    }
    if ($icon == false && !is_array($icon)) {
        $template =
            "<" .
            $element .
            " class='" .
            $classl .
            " " .
            $color .
            "'>" .
            $text .
            "</" .
            $element .
            ">";
    }

    return $template;
}
$elDate = HtmlEl("16/04/2021", "date", "", "");
$eldomain = HtmlEl("antoineeurtest.gr", "date", "", "");
$eltimezone = HtmlEl("(Europe/Athens)", "timezone", "", "");
$elbutton = HtmlEl("Συγχρονισμός Τώρα", "timezone", "", "a");
$elnserver = HtmlEl(".antoineeurtest.gr", "date", "ice", "");
$ellink = HtmlEl("300127 - CITIZEN", "date", "blue", "a");

$valueArr = [
    [
        [$elDate, $eltimezone],
        [$elDate, $eltimezone],
        [$eldomain],
        [$elDate, HtmlEl("Συγχρονισμός Τώρα", "button", "green2", "a")],
    ],
    [
        [HtmlEl("300127 - CITIZEN", "date", "blue", "a", true)],
        [HtmlEl("14610 - John Dooe", "date", "blue", "a", true)],
        [HtmlEl("26743 - George Papadogiannakis", "date", "blue", "a", true)],
        [HtmlEl("-", "date", "", "", true)],
        [buttonBottom()],
    ],
    [
        [
            HtmlEl("antoineeurtest.gr", "date", "", ""),
            HtmlEl(".antoineeurtest.gr", "date", "light_grey", "", [
                "icon-fi-sr-pencil",
                "icon-fi-sr-pencil",
            ]),
        ],
        [
            HtmlEl("antoineeurtest.gr", "date", "", ""),
            HtmlEl(".antoineeurtest.gr", "date", "light_grey", "", true),
        ],
    ],
    [
        [HtmlEl("Εισγαγωγή Νέων Εξυπηρετητών", "date", "", "", true)],
        [HtmlEl("ns1.free-host-ns-test.gr", "date", "", "", true)],
        [HtmlEl("ns1.free-host-ns-test.gr", "date", "", "", true)],
        [HtmlEl("ns1.free-host-ns-test.gr", "date", "", "", true)],
        [buttonBottom()],
    ],
];

$formArr = [
    ["", "", "", ""],
    [
        selectEl(
            [
                "300146 ‐ Κωνσταντίνος Χριστόπουλος",
                "300134 ‐ pantazis vastardis",
            ],
            "Κάτοχος"
        ),
        selectEl(
            [
                "300146 ‐ Κωνσταντίνος Χριστόπουλος",
                "300134 ‐ pantazis vastardis",
            ],
            "Κάτοχος"
        ),
        selectEl(
            [
                "300146 ‐ Κωνσταντίνος Χριστόπουλος",
                "300134 ‐ pantazis vastardis",
            ],
            "Κάτοχος"
        ),
        selectEl(
            [
                "300146 ‐ Κωνσταντίνος Χριστόπουλος",
                "300134 ‐ pantazis vastardis",
            ],
            "Κάτοχος"
        ),

        /*inputEl('test','Οικονομικά','text','','required nameservers'),*/
    ],
    [formPersonal($formPersonalArr), formPersonal($formPersonalArr)],
    [
        select2($nservers, "pant"),
        selectEl(["test", "test", "test"], "test"),
        selectEl(["test", "test", "test"], "test"),
        selectEl(["test", "test", "test"], "test"),
    ],
];

//inputEl('test','email','text','','required email'),
//inputEl('test','domain','text','','required domain'),
//inputEl('test','nameservers','text','','required nameservers'),
///inputEl('test','telephone','text','','required telephone'),
//inputEl('test','postcode','text','min4 max6','required postcode'),
//inputEl('test','password','password','','required password'),

$domaininfo = [
    [
        "title" => "Πληροφορίες",
        "text" => [
            "Ημ/νία κατοχύρωσης :",
            "Ημ/νία λήξης :",
            "Δέσμη ονομάτων :",
            "Τελευταίος συγχρονισμός :",
        ],
    ],
    [
        "title" => "Πρόσωπα Επαφής",
        "text" => [
            "Κάτοχος :",
            "Τεχνικός :",
            "Διαχειριστής :",
            "Οικονομικά :",
            "",
        ],
    ],
    [
        "title" => "Προσωπικοί Εξυπηρετητές",
        "text" => ["Εξυπηρετητής :", "Εξυπηρετητής :"],
    ],
    [
        "title" => "Εξυπηρετητές Ονοματοδοσίας",
        "text" => [
            "Διαθέσιμες Επιλογές :",
            "Εξυπηρετητής :",
            "Εξυπηρετητής :",
            "Εξυπηρετητής :",
            "",
        ],
    ],
];

$page_title = "antoineeurtest.gr";

$breadcrumb = ["Dashboard ", "Users", "An Ubavicius", "Domains"];

$tabs = [["name" => "Στοιχεία Domain"], ["name" => "Πρόσθετες Υπηρεσίες"]];
?>

 ?>