(function () {
	...
	function gridRefresh() {
		if (typeof grid != "undefined") {
			...
			navbarleft.addEventListener("webkitTransitionEnd", function () {
  ...
  */
	...
	function removeRow(el) {
		$(el).click(function () {
			...
			if (addActionObj.numOfRow > 1) {
				...
				if (
					addActionObj.parent.find(".info-form[data-collapsed='false']")
						.length == 0
				) {
	...
	function countRow() {
		addActionObj.numOfRow = addActionObj.parent.find(
			".info-row:not(.hide):not(.last)"
		).length;
	}
	...
	window.makeElement = function (el) {
		...
		addActionObj.cloneEL = addActionObj.parent
			.find(".info-row.hide")
			.clone()
			.removeClass("hide");
		addActionObj.removeEl = () => {
			removeRow(addActionObj.cloneEL.find(".info-icon.action0"));
		};
		addActionObj.numOfRow = addActionObj.parent.find(
			".info-row:not(.hide):not(.last)"
		).length;
...
//333
  function loop(el,index){
	...
	el.left=amount;
	if(index>0 && arrMessege[index-1]['left']<100 ){
		return
	}
	if (amount < el.width()) { 
		dontstopanimetion = true;	  
	   el.css("left",(amount+20)+"px");
	}else{		
		elHeight= el.height();
	...
	if (elHeight <= 0){		
		dontstopanimetion = false;	
		$(el[0]).remove();
		arrMessege.splice(el.arrIndex, 1);
	}
}
...
//--------------------------------------------------------------------------------------
	...
	function noScrollToBody(isTrue) {
		var hasScrollBar =
			messageCont[0].scrollHeight > messageCont[0].clientHeight ? true : false;
	...
	window.bindinput = {
		val: (el) => {
			$(".search-top,.search-mobile").val($(el).val());
		},
	};
